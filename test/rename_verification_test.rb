...
#    - No old recording_studio_youtube references in Ruby files
#    - No old directories (lib/recording_studio_youtube, app/controllers/recording_studio_youtube, etc.)
...
class RenameVerificationTest < Minitest::Test
  ...
  # These tests check that the original "recording_studio_youtube" name has been
  # completely replaced. They only run after a rename has occurred.

  def test_no_old_gem_template_references_in_ruby_files
    # Skip if current name IS recording_studio_youtube (nothing to check - hasn't been renamed yet)
    skip if @gem_name == "recording_studio_youtube"
    ...
    ruby_files.each do |file|
      ...
      if content.include?("recording_studio_youtube") || content.include?("RecordingStudio::YouTube")
        files_with_old_refs << file
      end
    end

    assert files_with_old_refs.empty?,
           "Found old 'recording_studio_youtube' references in:\n#{files_with_old_refs.join("\n")}"
  end
  ...
  def test_no_old_gem_template_directories
    ...
    old_dirs = [
      File.join(@root, "lib", "recording_studio_youtube"),
      File.join(@root, "app", "controllers", "recording_studio_youtube"),
      File.join(@root, "app", "views", "recording_studio_youtube")
    ]

    existing_old_dirs = old_dirs.select { |d| Dir.exist?(d) }

    assert existing_old_dirs.empty?,
           "Found old 'recording_studio_youtube' directories:\n#{existing_old_dirs.join("\n")}"
  end

  def test_no_old_gemspec_file
    skip if @gem_name == "recording_studio_youtube"

    old_gemspec = File.join(@root, "recording_studio_youtube.gemspec")
  ...
  def test_no_old_main_lib_file
    ...
    old_lib = File.join(@root, "lib", "recording_studio_youtube.rb")
  ...
  def test_readme_has_no_leftover_template_identity_after_rename
    ...
    refute_includes content, "RecordingStudio::YouTube",
                    "README.md still contains leftover RecordingStudio::YouTube identity"
    refute_match %r{bowerbird-app/recording_studio_youtube(?:["'/]|$)}, content,
                 "README.md still points at bowerbird-app/recording_studio_youtube"
    refute_includes content, "https://github.com/bowerbird-app/RecordingStudio_youtube",
  ...
  def test_changelog_has_no_leftover_template_identity_after_rename
    ...
    refute_includes content, "RecordingStudio::YouTube",
                    "CHANGELOG.md still contains leftover RecordingStudio::YouTube identity"
    refute_match %r{bowerbird-app/recording_studio_youtube(?:["'/]|$)}, content,
                 "CHANGELOG.md still points at bowerbird-app/recording_studio_youtube"
    refute_includes content, "https://github.com/bowerbird-app/RecordingStudio_youtube",
  ...
  def test_gemspec_homepage_has_no_leftover_template_identity_after_rename
    ...
    content = read_gemspec
    refute_includes content, "RecordingStudio::YouTube",
                    "Gemspec still contains leftover RecordingStudio::YouTube identity"
    refute_match %r{bowerbird-app/recording_studio_youtube(?:["'/]|$)}, content,
                 "Gemspec still points at bowerbird-app/recording_studio_youtube"
    refute_includes content, "https://github.com/bowerbird-app/RecordingStudio_youtube",

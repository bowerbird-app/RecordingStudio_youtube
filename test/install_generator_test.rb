...
require "fileutils"
require "tmpdir"
require "generators/recording_studio_youtube/install/install_generator"

class InstallGeneratorTest < Minitest::Test
  INSTALL_TEMPLATE_PATH = File.expand_path(
    "../lib/generators/recording_studio_youtube/install/templates/INSTALL.md",
    __dir__
  )
  ...
  def build_generator(destination_root, options = {})
    RecordingStudio::YouTube::Generators::InstallGenerator.new(
      [],
      options,
  ...
  def test_mount_engine_uses_configured_mount_path
    ...
    assert_equal ["mount RecordingStudio::YouTube::Engine, at: \"/addons/recording\""], routes
  end
  ...
  def test_add_tailwind_source_does_not_duplicate_existing_entries
    with_temp_app do |dir|
      ...
      Rails.stub(:root, Pathname.new(dir)) do
        lines = generator.send(:tailwind_source_lines).map(&:last)
        File.write(css_path, ["@import \"tailwindcss\";", *lines, ""].join("\n"))
        generator.stub(:say, nil) { generator.add_tailwind_source }
        css = File.read(css_path)
        lines.each { |line| assert_equal 1, css.scan(/#{Regexp.escape(line)}/).size }
      end
    end
  end
  ...
  def test_add_tailwind_source_reports_missing_tailwind_config
    with_temp_app do |dir|
      ...
      Rails.stub(:root, Pathname.new(dir)) do
        ...
        assert_tailwind_notice(messages, generator)
      end
  ...
  def test_install_guide_includes_migration_and_host_setup_steps
    ...
    assert_includes install_guide, "bin/rails generate recording_studio_youtube:migrations"
  ...
  def assert_concrete_tailwind_sources(dir, css, generator)
    lines = generator.send(:tailwind_source_lines).map(&:last)
    assert_includes lines.join, "app/views"
    assert_includes lines.join, "app/components"
    lines.each do |line|
      assert_includes css, line
      refute_includes line, "*"
      relative = line[/^@source "(.*)";\z/, 1]
      resolved = File.expand_path(relative, File.join(dir, "app/assets/tailwind"))
      assert File.directory?(resolved), resolved
    end
    refute_includes css, "vendor/bundle/**"
    refute_includes css, "usr/local/bundle"
  end

  def assert_tailwind_notice(messages, generator)
    generator.send(:tailwind_source_lines).each do |entry|
      assert_includes messages, ["  #{entry.last}", :yellow]
    end
  end
end

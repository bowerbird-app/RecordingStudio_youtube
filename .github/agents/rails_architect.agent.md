...
**Engine Isolation**: Respect namespace isolation. All code should be properly namespaced under the engine's module (`RecordingStudio::YouTube`). Use `isolate_namespace` patterns correctly.
...
- **Browser Simulation**: When modifying UI or flows (like `dashboard/index.html.erb`), explicitly verify the user experience:
  ...
  2. Navigate to the relevant page (e.g., `/recording_studio_youtube` or `/recording_studio_youtube/analytics`).
...
When generating code, include file paths in comments (e.g., `# app/models/recording_studio_youtube/error_log.rb`).
...
**Configuration**:
- Provide a configuration block: `RecordingStudio::YouTube.configure do |config|`
